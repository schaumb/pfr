...
#include <boost/pfr/detail/integral_proxy.hpp>
...
constexpr decltype(auto) tie_as_tuple_get_if_proxy(Getter&& getter, Setter&& setter) {
    using type = std::invoke_result_t<Getter, Index, std::false_type>;
    if constexpr (std::is_integral_v<type>) {
        return integral_proxy{[getter] (auto ref) { return getter(Index{}, ref); },
                              [setter] (auto&& param) { return setter(Index{}, std::forward<decltype(param)>(param)); }};
    } else {
        return getter(Index{}, std::true_type{});
    }
}

template<class Getter, class Setter, std::size_t ... Ix>
constexpr auto tie_as_tuple(Getter&& getter, Setter&& setter, std::index_sequence<Ix...>) {
    return sequence_tuple::tuple<decltype(tie_as_tuple_get_if_proxy<size_t_<Ix>>(getter, setter))...>{
        tie_as_tuple_get_if_proxy<size_t_<Ix>>(getter, setter)...
    };
}
...
constexpr auto tie_as_tuple(T& val) noexcept {
  ...
  return tie_as_tuple(std::move(getter), std::move(setter), std::make_index_sequence<fields_count_tag{}>{});
}
